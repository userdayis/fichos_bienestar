<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">Editar Usuario: {{ $usuario->name }}</h2>
    </x-slot>
        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-theme-panel rounded-2xl border border-white/10 shadow-xl p-8">
                ...
                <form method="POST" action="{{ route('admin.usuarios.update', $usuario) }}">
                    ...
                    <div class="mb-5">
                        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Contraseña <span class="text-gray-600">(dejar vacío para no cambiar)</span></label>
                        ...
                        <div>
                            ...
                            <select name="actividad_id" class="w-full bg-theme-bg border border-white/20 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-theme-mustard">
                                ...
                                @foreach($actividades as $act)
                                    <option value="{{ $act->id }}" {{ old('actividad_id', $usuario->actividad_id) == $act->id ? 'selected' : '' }}>{{ $act->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
